...
export default function AdminArtikel() {
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      return res.data.url; // contoh: "/uploads/namafile.jpg"
    } catch (err) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (file) {
        ...
        if (uploadedUrl) {
          imageUrl = uploadedUrl; // disimpan sebagai '/uploads/...'
        } else {
          return;
        }
      }
    ...
    <div className="p-4 max-w-4xl mx-auto">
      ...
      <div className="bg-white p-4 rounded shadow mb-6">
        ...
        <div className="mt-4">
          ...
          {editId && (
            <button className="text-gray-600" onClick={() => setEditId(null)}>Batal</button>
          )}
        </div>
      </div>
